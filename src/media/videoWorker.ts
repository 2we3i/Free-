...
export async function getManualClip(
  runId: string,
  channelLabel: string,
  videoPrompt: string,
): Promise<Buffer> {
  await sendVideoPrompt(runId, channelLabel, videoPrompt);

  logger.info({ runId, channelLabel }, 'waiting for manually generated clip from Telegram');
